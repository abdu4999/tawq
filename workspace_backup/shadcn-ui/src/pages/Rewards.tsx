...
export default function Rewards() {
  ...
  const loadRewardsData = async () => {
    try {
      ...
      // حساب النقاط من المهام المكتملة
      const completedTasks = tasks.filter(t => t.status === 'completed');
      const points = completedTasks.length * 15;
      setUserPoints(points);

      // إنشاء تحديات من المهام
      const generatedChallenges: Challenge[] = [
        {
          id: '1',
          title: 'إكمال 10 مهام هذا الأسبوع',
          description: 'أكمل 10 مهام للحصول على 150 نقطة',
          points: 150,
          difficulty: 'medium',
      ...
      // بيانات تجريبية للتحديات والمكافآت الإضافية
      const currentEmployee = users[0];
      if (currentEmployee) {
