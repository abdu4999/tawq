...
export class ErrorStorage {
  ...
  async logError(error: Omit<ErrorLog, 'id' | 'timestamp'>): Promise<string> {
    try {
      ...
      // Store in Supabase if available and table exists
      try {
        // First check if the error_logs table exists by trying to query it
        const { error: tableCheckError } = await supabase
          .from('error_logs')
          .select('id')
          .limit(1);

        // If no table error, proceed with insert
        if (!tableCheckError) {
          ...
          console.warn('Error logs table does not exist in Supabase, using memory storage only');
        }
      } catch (dbError) {
