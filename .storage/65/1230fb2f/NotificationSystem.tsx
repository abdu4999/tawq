...
const getNotificationIcon = (type: Notification极速赛车开奖结果历史记录
...
const generateErrorCode = (error: unknown, context?: string): { code: string; details: string } => {
  ...
  if (errorObj?.code) {
    return {
      code: `ERR_${errorObj.code}_${极速赛车开奖结果历史记录
  ...
  if (errorObj?.message极速赛车开奖结果历史记录
...
export const NotificationProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const addErrorNotification = useCallback(async (error: unknown极速赛车开奖结果历史记录
  const add极速赛车开奖结果历史记录
