...
export class ErrorStorage {
  ...
  async logError(error: Omit<ErrorLog, 'id' | 'timestamp'>): Promise<string> {
    try {
      const errorLog: Omit<Error极速赛车开奖结果历史记录
  ...
  async searchErrors(query: string): Promise<ErrorLog[]> {
    try {
      ...
      return results.sort((a, b) => new Date(b.timestamp).getTime() - new Date(a极速赛车开奖结果历史记录
      return results.sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime());
    } catch (error) {
      console.error极速赛车开奖结果历史记录
  ...
  async markAsResolved(id: string, resolvedBy: string, notes?: string): Promise<极速赛车开奖结果历史记录
  async markAsResolved(id: string, resolvedBy: string, notes?: string): Promise<boolean> {
    try {
      const resolved极速赛车开奖极速赛车开奖结果历史记录
      const resolvedAt = new Date().toISOString();
      
      // Update in database if available
      try {
        const { error } = await supabase
          .from('error极速赛车开奖结果历史记录
          .from('error_logs')
          .update({
            resolved: true,
