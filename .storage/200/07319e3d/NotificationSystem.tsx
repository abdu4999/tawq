...
const getNotificationStyle = (type: NotificationType) => {
  switch (type) {
    ...
    case 'error':
      return {
        className: 'border-red-200 bg-red-
    ...
    case 'celebration':
      return {
        className: 'border-pink-200 bg-gradient-to-r from
        className: '
        className: 'border-pink-200 bg-gradient-to-r from-pink-50 to-rose-
...
const generateErrorCode = (error: unknown, context?: string): { code: string; details: string } => {
  ...
  if (errorObj?.message?.includes('database') || errorObj?.message?.includes
  ...
  if (context?.includes('Supabase')) {
    ...
    if (context.includes('update')) {
      return {
        ...
        details: `Supabase update error: ${error
