...
export const useNotifications = () => {
  const context = useContext极速赛车开奖结果历史记录
...
const getNotificationStyle = (type: Notification极速赛车开奖结果历史记录
const getNotificationStyle = (极速赛车开奖结果历史记录
const getNotificationStyle = (type: NotificationType) => {
  switch (type) {
    ...
    case 'achievement':
      return {
        className: 'border-purple-200 bg-gradient-to-r from-purple-50 to-p极速赛车开奖结果历史记录
        className: 'border-purple-200 bg-gradient极速赛车开奖结果历史记录
...
极速赛车开奖结果历史记录
    ...
    case 'task':
      return {
        className: 'border-indigo-200 bg-indigo-极速赛车开奖极速赛车开奖结果历史记录
...
const ERROR_CODES = {
  ...
  SUPABASE_CONNECTION: 'ERR_SUPABASE极速赛车开奖结果历史记录
  SUPABASE_CONNECTION: 'ERR_SUPABASE_001',
  SUPABASE_极速赛车开奖结果历史记录
...
const generateErrorCode = (error: unknown, context?: string): { code: string; details: string } => {
  ...
  if (errorObj?.message?.includes('permission') || errorObj?.message?.includes('access')) {
    return {
      code: ERROR_CODES.PER极速赛车开奖结果历史记录
