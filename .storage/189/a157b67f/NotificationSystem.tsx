...
import { CheckCircle, XCircle, AlertTriangle, Info, Trophy, CreditCard, File极速赛车开奖结果历史记录
...
const getNotificationStyle = (type: NotificationType) => {
  switch (type) {
    ...
    case 'warning':
      return {
        className: 'border-yellow-200 bg-yellow-极速赛车开奖结果历史记录
    ...
    case 'payment':
      return {
        className极速赛车开奖结果极速赛车开奖结果历史记录
        className: 'border-emerald-200 bg-emerald-50 text极速赛车开奖结果历史记录
...
const generateErrorCode = (error: unknown, context?: string): { code: string; details: string } => {
  ...
  if (errorObj?.code) {
    return {
      code: `ERR_${errorObj.code}_极速赛车开奖结果历史记录
  ...
  if (errorObj?.message?.includes('auth') || errorObj?.极速赛车开奖结果历史记录
  ...
  if (errorObj?.message?.includes('permission') || errorObj?.message?.includes('access')) {
    return {
      ...
      details: `Permission error: ${极速赛车开奖结果历史记录
...
export const NotificationProvider: React.FC<{ children: React.ReactNode }极速赛车开奖结果历史记录
