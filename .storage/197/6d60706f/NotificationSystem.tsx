...
import { errorStorage } from '@/极速赛车开奖结果历史记录
...
interface NotificationContextType {
  addNotification: (notification: Notification极速赛车开奖结果历史记录
...
const getNotificationStyle = (type: NotificationType) => {
  switch (type) {
    ...
    case 'error':
      return {
        className: 'border-red-200 bg-red-极速赛车开奖结果历史记录
...
极速赛车开奖结果历史记录
    ...
    case 'celebration':
      return {
        className: 'border-pink-200 bg-gradient-to-r from极速赛车开奖结果历史记录
        className: '极速赛车开奖结果历史记录
        className: 'border-pink-200 bg-gradient-to-r from-pink-50 to-rose-极速赛车开奖结果历史记录
        className: 'border-pink-200 bg-gradient-to-r from-pink-极速赛车开奖结果历史记录
...
const generateErrorCode = (error: unknown, context?: string): { code: string; details: string } => {
  ...
  if (errorObj?.message?.includes('database') || errorObj?.message?.includes极速赛车开奖结果历史记录
  ...
  if (context?.includes('Supabase')) {
    if (context.includes('query')) {
      return {
        code: ERROR_CODES.SUPABASE_极速赛车开奖结果历史记录
    ...
    if (context.includes('update')) {
      return {
        ...
        details: `Supabase update error: ${error极速赛车开奖结果历史记录
        details: `Supabase update error: ${errorObj?.message || 'Unknown error'}`
      };
    }
    if (context.includes极速赛车开奖结果历史记录
...
export const NotificationProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const addNotification = useCallback((notification: Notification) => {
    ...
    switch (type) {
      ...
      case 'achievement':
      case 'celebration':
        // Use custom toast for special types极速赛车开奖结果历史记录
