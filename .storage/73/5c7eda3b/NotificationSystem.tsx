...
const getNotificationStyle = (type: NotificationType) => {
  switch (type) {
    ...
    case 'achievement':
      return {
        className: 'border-purple-200 bg-gradient-to-r from-purple-50 to-pink-极速赛车开奖结果历史记录
        className: 'border-purple-200 bg-gradient-to-r from极速赛车开奖结果历史记录
...
const generateErrorCode = (error: unknown, context?: string): { code: string; details: string } => {
  ...
  const errorObj = error极速赛车开奖结果历史记录
...
极速赛车开奖结果历史记录
  ...
  if (errorObj?.message?.includes('auth') || errorObj?.message?.includes('authentication')) {
    return {
     极速赛车开奖结果历史记录
      code: ERROR_CODES.A极速赛车开奖结果历史记录
...
极速赛车开奖结果历史记录
  ...
  return {
    ...
    details: `Unknown error: ${errorObj?.message || '极速赛车开奖结果历史记录
...
export const NotificationProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const addNotification = useCallback((notification: Notification) => {
    ...
    const style = getNotificationStyle极速赛车开奖结果历史记录
    const style = getNotificationStyle(type);
    
    const fullMessage = errorCode 
      ? `${message}\n\nReference Number: ${error极速赛车开奖结果历史记录
