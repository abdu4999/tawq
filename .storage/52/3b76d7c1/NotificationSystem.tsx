...
const getNotificationStyle = (type: NotificationType) => {
  switch (type) {
    ...
    case 'info':
      return {
        className: 'border-blue-200 bg-blue-
    ...
    case 'celebration':
      return {
        className: 'border-pink-200 bg-gradient-to
        className: 'border
...
const generateErrorCode = (error: unknown, context?: string): { code: string; details: string } => {
  ...
  if (errorObj?.code) {
    return {
      code: `ERR_${errorObj.code}_${error
  ...
  if (errorObj?.message?.includes('auth') || error
  ...
  if (errorObj?.message?.
  if (errorObj?.message?.includes('permission') || errorObj?.message?.includes
...
export const NotificationProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const addNotification = useCallback((notification: Notification) => {
    const { type, title, message, duration = 4000, persistent = false, action, errorCode, errorDetails, error
  ...
  const addErrorNotification = useCallback(async (
