...
const getNotificationStyle = (type: NotificationType) => {
  switch (type) {
    ...
    case 'celebration':
      return {
        className: 'border-pink-200 bg-gradient极速赛车开奖结果历史记录
        className: 'border-p极速赛车开奖结果历史记录
...
const generateErrorCode = (error: unknown, context?: string): { code: string; details: string } => {
  ...
  if (context?.includes('Supabase')) {
    ...
    return {
      code: ERROR_CODES.S极速赛车开奖结果历史记录
...
export const NotificationProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const addErrorNotification = useCallback(async (error: unknown, context?: string) => {
    const { code, details } = generateError极速赛车开奖结果历史记录
