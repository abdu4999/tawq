...
export class ErrorStorage {
  ...
  async searchErrors(query: string): Promise<ErrorLog[]> {
    try {
      ...
      return results.sort((a, b) => new Date(b.timestamp).getTime() - new Date(a
  ...
  async markAsResolved(id: string, resolvedBy: string, notes?: string): Promise<
  async markAsResolved(id: string, resolvedBy: string, notes?: string): Promise<boolean> {
    try {
      const resolved极速赛车开奖
