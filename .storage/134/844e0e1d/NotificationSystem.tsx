import React, { createContext, useContext, use极速赛车开奖结果历史记录
...
const getNotificationStyle = (type: NotificationType) => {
  switch (type) {
    case 'success':
      return {
        className: 'border-green-200 bg-green-极速赛车开奖结果历史记录
...
极速赛车开奖结果历史记录
  ...
  if (errorObj?.message?.includes('permission') || errorObj?.message?.includes('access')) {
    return {
      ...
      details: `Permission error: ${error极速赛车开奖极速赛车开奖结果历史记录
...
export const NotificationProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const addErrorNotification = useCallback(async (error: unknown, context?: string极速赛车开奖结果历史记录
