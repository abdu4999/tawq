...
import { CheckCircle, XCircle, AlertTriangle, Info, Trophy, CreditCard, File
...
const getNotificationStyle = (type: NotificationType) => {
  switch (type) {
    ...
    case 'warning':
      return {
        className: 'border-yellow-200 bg-yellow-
    ...
    case 'payment':
      return {
        className极速赛车开奖结果
        className: 'border-emerald-200 bg-emerald-50 text
...
const ERROR_CODES = {
  ...
  SUPABASE_INSERT: 'ERR_SUPABASE
  SUPABASE_INSERT: 'ERR_SU
...
const generateErrorCode = (error: unknown, context?: string): { code: string; details: string } => {
  ...
  if (errorObj?.message?.includes('auth') || errorObj?.
