...
const getNotificationStyle = (type: NotificationType) => {
  switch (type) {
    case 'success':
      return {
        className: 'border-green-200 bg-green-50 text-left',
    ...
    case 'warning':
      return {
        className: 'border-yellow-200 bg-yellow-50 text-left',
        iconColor: 'text-yellow-600'
      };
    case 'info':
      return {
        className: 'border-blue-200 bg-blue-50 text-left',
        iconColor: 'text-blue-600'
      };
    case 'achievement':
      return {
        className: 'border-purple-200 bg-gradient-to-r from-purple-50 to-pink-50 text-left',
        iconColor: 'text-purple-600'
      };
    case 'payment':
      return {
        className: 'border-emerald-200 bg-emerald-50 text-left',
        iconColor: 'text-emerald-600'
      };
    case 'task':
      return {
        className: 'border-indigo-200 bg-indigo-50 text-left',
        iconColor: 'text-indigo-600'
      };
    case 'celebration':
      return {
        className: 'border-pink-200 bg-gradient-to-r from-pink-50 to-rose-50 text-left',
        iconColor: 'text-pink-600'
      };
    default:
      return {
        className: 'border-gray-200 bg-gray-50 text-left',
...
export const NotificationProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const addNotification = useCallback((notification: Notification) => {
    ...
    // For error notifications, show only reference number to user
    const userMessage = errorCode 
      ? `حدث خطأ في النظام. الرجاء تدوين الرقم المرجعي للإبلاغ.\\n\\nالرقم المرجعي: ${errorId || errorCode}`
      : message;

    const toastOptions = {
      duration: persistent ? Infinity : duration,
      className: `${style.className} border-2 shadow-lg text-left`,
  ...
  const addErrorNotification = useCallback(async (error: unknown, context?: string) => {
    ...
    // Store error in storage with full details
    ...
    // Show user only the reference number, not error details
    addNotification({
      type: 'error',
      title: 'خطأ في النظام',
      message: 'حدث خطأ في النظام. الرجاء تدوين الرقم المرجعي للإبلاغ.',
      errorCode: code,
      errorId: errorId,
      persistent: true
    });
