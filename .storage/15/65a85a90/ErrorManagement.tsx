...
export default function ErrorManagement() {
  ...
  const handleSearch = async () => {
    try {
      ...
      addNotification({
        type: 'error',
        title: 'Search Error',
        message: 'Failed to search error logs'
      });
    } finally {
      setLoading(false);
    }
  ...
  const handleResolve = async (errorId: string) => {
    try {
      const success = await errorStorage.markAsResolved(errorId, 'admin', 'Manually resolved');
      if (success) {
        addNotification({
          type: 'success',
          title: 'Resolved',
  ...
  const exportErrors = () => {
    const csvContent = [
      ['Reference Number', 'Error Code', 'Message', 'Details', 'Status', 'Occurrence Time', 'Resolution Time', 'Resolution Notes'],
      ...errors.map(error => [
        error.id,
        error.error_code,
        error.error_message,
        ...
        error.resolved_at ? new Date(error.resolved_at).toLocaleString('en-US') : '',
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 flex" dir="ltr">
      <Sidebar />
      ...
      <div className="flex-1 ml-80 p-6 space-y-6">
        <div className="text-center space-y-4">
          ...
          <p className="text-xl text-gray-600">System error tracking and management</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <Card className="bg-gradient-to-br from-blue-500 to-blue-600 text-white border-0">
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-blue-100 text-sm">Total Errors</p>
          ...
          <Card className="bg-gradient-to-br from-green-500 to-green-600 text-white border-0">
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-green-100 text-sm">Resolved Errors</p>
          ...
          <Card className="bg-gradient-to-br from-red-500 to-red-600 text-white border-0">
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-red-100 text-sm">Unresolved Errors</p>
        ...
        <Card>
          ...
          <CardContent>
            <div className="flex gap-4">
              <Input
                placeholder="Search by reference number, error code, or message..."
                value={searchQuery}
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>List of all recorded system errors</CardDescription>
          </CardHeader>
          <CardContent>
            {loading ? (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-gray-500">Time: {formatDate(error.timestamp)}</p>
                        {error.context && (
        ...
        {selectedError && (
          <Card>
            <CardHeader>
              ...
              <CardDescription>Detailed information about the selected error</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="text-sm font-medium text-gray-700">Occurrence Time</label>
                ...
                {selectedError.resolved && (
                  <div className="space-y-2">
                    <label className="text-sm font-medium text-gray-700">Resolution Info</label>
                    <p>Resolved by: {selectedError.resolved_by}</p>
                    <p>Resolution time: {formatDate(selectedError.resolved_at!)}</p>
                    {selectedError.resolution_notes && (
                      <p>Resolution notes: {selectedError.resolution_notes}</p>
                    )}
                  </div>
                )}
