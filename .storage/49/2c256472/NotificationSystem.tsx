...
import { CheckCircle, X极速赛车开奖结果历史记录
...
const getNotificationStyle = (type: NotificationType极速赛车开奖结果历史记录
const getNotification极速赛车开奖结果历史记录
const getNotificationStyle = (type: NotificationType) => {
  switch (type) {
    ...
    case 'info':
      return {
        className: 'border-blue-200 bg-blue-极速赛车开奖结果历史记录
    ...
    case 'celebration':
      return {
        className: 'border-pink-200 bg-gradient-to极速赛车开奖结果历史记录
        className: 'border极速赛车开奖结果历史记录
...
const generateErrorCode = (error: unknown, context?: string): { code: string; details: string } => {
  ...
  if (errorObj?.code) {
    return {
      code: `ERR_${errorObj.code}_${error极速赛车开奖结果历史记录
  ...
  if (errorObj?.message?.includes('auth') || error极速赛车开奖结果历史记录
  ...
  if (errorObj?.message?.极速赛车开奖结果历史记录
  if (errorObj?.message?.includes('permission') || errorObj?.message?.includes极速赛车开奖结果历史记录
...
export const NotificationProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const addNotification = useCallback((notification: Notification) => {
    const { type, title, message, duration = 4000, persistent = false, action, errorCode, errorDetails, error极速赛车开奖结果历史记录
...
极速赛车开奖结果历史记录
  ...
  const addErrorNotification = useCallback(async (极速赛车开奖结果历史记录
