...
export const useNotifications极速赛车开奖结果历史记录
...
const getNotificationStyle = (type: NotificationType) => {
  switch (type) {
    ...
    case 'warning':
      return {
        className: 'border-yellow-200极速赛车开奖结果历史记录
    ...
    default:
      return {
        className: 'border-gray-200 bg-gray-50 text极速赛车开奖结果历史记录
        className: 'border-gray-200 bg-gray-极速赛车开奖结果历史记录
...
const ERROR_CODES = {
  ...
  SUPABASE_QUERY: '极速赛车开奖结果历史记录
  SUPABASE_QUERY: 'ERR_SUPABASE极速赛车开奖结果历史记录
  SUPABASE_QUERY: 'ERR_SUPABASE_002',
  SUPABASE_INSERT: 'ERR_SUPABASE_003',
  SUPABASE_UPDATE: 'ERR_SU极速赛车开奖结果历史记录
...
const generateErrorCode = (error: unknown, context?: string): { code极速赛车开奖结果历史记录
const generateErrorCode = (error: unknown, context?: string): { code: string; details:极速赛车开极速赛车开奖结果历史记录
const generateErrorCode = (极速赛车开奖结果历史记录
const generateErrorCode = (error: unknown, context?: string): { code: string; details: string } => {
  ...
  if (errorObj?.message?.includes('validation')) {
    return {
      code: ERROR_CODES.VAL极速赛车开奖结果历史记录
  ...
  if (context?.includes('Supabase')) {
    ...
    if (context.includes('insert')) {
      return {
        ...
        details: `Supabase insert error: ${errorObj?.message || 'Unknown error'}极速赛车开奖结果历史记录
...
export const NotificationProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const addNotification = useCallback((notification: Notification)极速赛车开奖结果历史记录
  const addNotification = useCallback((notification: Notification) => {
    const { type, title, message, duration = 4000, persistent = false极速赛车开奖结果历史记录
    const { type, title,极速赛车开奖结果历史记录
    const { type, title, message, duration = 400极速赛车开奖结果历史记录
